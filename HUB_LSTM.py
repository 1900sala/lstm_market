...
def norm_everyday(data):
    for i in range(len(data)):
        mean_price = np.mean(data[i][:200, 0])
        std_price = np.std(data[i][:200, 0])
        mean_volume = np.mean(data[i][:200, 1])
        std_volume = np.std(data[i][:200, 1])
        mean_ba = np.mean(data[i][:200, 13:])
        std_ba = np.std(data[i][:200, 13:])
